...
# Deployment Slot configuration for test-api
resource "azurerm_app_service_slot" "test_api_deployment_slot" {
  name                = "test-api-${var.stage}-deployment-slot"
  location            = azurerm_resource_group.rg.location
  resource_group_name = azurerm_resource_group.rg.name
  app_service_plan_id = azurerm_app_service_plan.main.id
  app_service_name    = azurerm_app_service.qra_api.name
